      ...
      if (document.getElementsByClassName('sub-menu')) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          subMenuToggle.addEventListener('click', function (e) {
            e.preventDefault();
            var targetId = subMenuToggle.getAttribute('aria-controls');
  ...
  Drupal.behaviors.mainMenu = {
    attach: function attach(context) {
      ...
      function setIsOpenBool() {
        if (context.getElementById('search__dropdown').classList.contains('search-dropdown-is-open')) {
          isOpenBool = true;
        } else {
          isOpenBool = false;
        }
      ...
      function openCloseSearch() {
        toggleClass(context.getElementById('search-trigger'), 'search-is-open');
        toggleClass(context.getElementById('search__dropdown'), 'search-dropdown-is-open');
        setIsOpenBool();
      }

      // Add click trigger to search element
      context.getElementById('search-trigger').addEventListener('click', function () {
        openCloseSearch();
      });

      // Add hover trigger to search element
      context.getElementById('search-trigger').addEventListener('mouseover', function () {
        if (!isOpenBool) {
          toggleClass(context.getElementById('search-trigger'), 'search-is-open');
          toggleClass(context.getElementById('search__dropdown'), 'search-dropdown-is-open');
          setIsOpenBool();
      ...
      var menuItems = context.getElementsByClassName('menu-item');
      ...
      for (var i = 0; i < menuItems.length; i++) {
        menuItems[i].addEventListener("mouseover", closeSearchOnHoverAway);
      }

      // Trigger search dropdown with spacebar or enter key
      context.getElementById('search-trigger').addEventListener('keyup', function (event) {
      ...
      var clickArea = context.getElementById('banner');
      var searchTrigger = context.getElementById('search-trigger');
      var searchBoxElement = context.getElementById('search__dropdown');
      ...
      context.body.addEventListener('keyup', function (e) {
        if (e.which === 9) /* tab */{
...
//Make sure header has been initialized
if (jQuery(".header").length) {

  var controller = new ScrollMagic.Controller();
  var skills_array = new Array();
  var headerBackgroundX;
  var headerBackgroundY;
  var headerBgPos = jQuery('.header').css('backgroundPosition');
  var headerBackgroundPos = headerBgPos ? headerBgPos.split(" ") : [];
  var animate_array = [
    {
      elem: ".header", 
      tween_name: "header-tween",
      speed: .5,
      from:[
          { bgpos: "5000px center",
        }],
      to:[
          { bgpos: getHeaderBackgroundPos(),
            delay: .5,
        }]
    },
    
    {
      elem: "h1.header__title", 
      tween_name: "header-h1-tween",
      speed: .5,
      from:[
          { opacity: 0,
            x:200,
    ...
    {
      elem: "h2.header__subtitle", 
      tween_name: "header-h2-tween",
      speed: .5,
      from:[
          { opacity: 0,
            x:200,
    ...
    {
      elem: ".header__description", 
      tween_name: "header-desc-tween",
      speed: .5,
      from:[
          { opacity: 0,
            x:200,
    ...
    {
      elem: ".header__social_buttons", 
      tween_name: "header-social-tween",
      speed: .4,
      from:[
          { opacity: 0,
            y:50,
  ...
  var tweenSkills = function() { 
      jQuery('.skills .skill').each(function(i) {
        TweenMax.to(this, .2, {
          y:0,
          opacity:1,
          ease: Power2.easeOut,
  ...
  var tweenVideo = function() { 
    TweenMax.to('.video', .4, {opacity:1, y:0});
  }

  // Animate Skills on scroll
  var scroll_skills = new ScrollMagic.Scene({
    triggerElement: '.header__description'
  })
  .setTween(tweenSkills);

  // Animate Video on scroll
  var scroll_video = new ScrollMagic.Scene({
    triggerElement: '.header__description'
  })
  .setTween(tweenVideo);
...
function init() {
  ...
  if(jQuery(".header").hasClass("header--is-animated")) {

    //Setup the header tweens
    setupTweens();

    //Now that the tweens are set, reset the opacity on the header
    TweenMax.set(".header", {opacity:1});

    //On resize clear inline styles set by tween
    jQuery( window ).resize(function() {
      jQuery('.header').css('backgroundPosition', "");
    });
  ...
  //Add to controller for scrolling  
  controller.addScene([
  scroll_skills,
  scroll_video
  ]);
...
function getHeaderBackgroundPos() {
  if (jQuery(".header").length) {
    var headerBackgroundPos = jQuery('.header').css('background-position').split(" ");
    
    if(headerBackgroundPos[0] == undefined || headerBackgroundPos[1] == undefined) {
      headerBackgroundX = jQuery('.header').css("background-position-x");
      headerBackgroundY = jQuery('.header').css("background-position-y");

      //Fallback for IE 11, 
      //If this STILL brings back undefined then we are in IE 11 land
      //and so we can serve default 50% 50%
      if(headerBackgroundX == undefined || headerBackgroundY == undefined) {
        headerBackgroundX = '50%';
        headerBackgroundY = '50%';
      }

    }

    else {
      headerBackgroundX = headerBackgroundPos[0];
      headerBackgroundY = headerBackgroundPos[1];
    }

    return(headerBackgroundX + ' ' + headerBackgroundY);
  }
}

function setupTweens() {
  for(i=0; i<animate_array.length; i++){
    var obj = animate_array[i];
    TweenMax.set(obj.elem, {
      delay: obj.from[0].delay,
      backgroundPosition: obj.from[0].bgpos,
      opacity: obj.from[0].opacity,
      x: obj.from[0].x,
      y: obj.from[0].y,
    });
  }
...
function setupSkills(){
  jQuery('.skills .skill').each(function(i) {
    TweenMax.set(this, { y:400, opacity:0});
  });

  TweenMax.set('.video', { opacity:0, y:400});
}

function tweenAnimate() {
  for(i=0; i<animate_array.length; i++){
    var obj = animate_array[i];
    TweenMax.to(obj.elem, obj.speed, {
      delay: obj.to[0].delay,
      backgroundPosition: obj.to[0].bgpos,
      opacity: obj.to[0].opacity,
      x: obj.to[0].x,
      y: obj.to[0].y,
      ease: Power3.easeOut,
    });
